...
public class ToolLaserGun extends TinkerToolCore implements cofh.redstoneflux.api.IEnergyContainerItem, IAEItemPowerStorage, IToggleTool<ToolLaserGun.Mode> {
    ...
    private static Optional<ItemStack> getActiveLaserGun(EntityLivingBase entity) {
        return Arrays.stream(EnumHand.values())
                ...
                .filter(stack -> stack != null && stack.getItem() instanceof ToolLaserGun && TagUtil.getTagSafe(stack).getInteger(ATTACK_DURATION_TAG) > 0)
                .findFirst();
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        Optional.ofNullable(IToggleTool.getMode(stack, Mode.class)).ifPresent(mode -> tooltip.add(
                I18n.format("msg.plustic.tool_mode", I18n.format(mode.getUnlocName()))));
        ...
        if (IToggleTool.getMode(itemStackIn, Mode.class) == Mode.ATTACK) {
            res = Optional.ofNullable(Utils.raytraceEntityPlayerLookWithPred(playerIn, range(itemStackIn), ent -> !(ent instanceof IEntityMultiPart)))
                    ...
                    .map(ent -> {
                        ...
                        // able to attack?
                        if (this.extractEnergy(itemStackIn, energyTaken, true) >= energyTaken && nbt.getInteger(ATTACK_DURATION_TAG) <= 0) {
    ...
    public EnumActionResult onItemUse(@Nonnull EntityPlayer player, World worldIn, @Nonnull BlockPos pos, @Nonnull EnumHand hand, @Nonnull EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (IToggleTool.getMode(stack, Mode.class) == Mode.TOOL) {
            ...
            if (!(smeltingRes = FurnaceRecipes.instance().getSmeltingResult(new ItemStack(state.getBlock(), 1, state.getBlock().getMetaFromState(state))).copy()).isEmpty()) {
                ...
                if (this.extractEnergy(stack, energyTaken, true) >= energyTaken && worldIn.destroyBlock(pos, false)) {
        ...
        public static void doRenderBeam(EntityLivingBase shooter) {
            getActiveLaserGun(shooter)
                    .ifPresent(stack -> {
                        ...
                        Vec3d vec0 = shooter.getPositionVector().add(new Vec3d(0, shooter.getEyeHeight() + 0.2, 0));
                        Vec3d vec1 = vec0;
