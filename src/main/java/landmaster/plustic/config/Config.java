...
public class Config extends Configuration {
    ...
    ItemStack fetchThing(Random random) {
        if (trashThingsSum <= 0) {
    ...
    public static boolean isCentrifugeRecipeValid(AlloyRecipe recipe) {
        Pair<String, Set<String>> pairToCheck = Pair.of(FluidRegistry.getFluidName(recipe.getResult()), recipe.getFluids().stream().map(FluidRegistry::getFluidName).collect(Collectors.toSet()));
    ...
    public void init1() {
                ...
                "Objects that the Trash modifier will generate; enter in the format \"weight|modid:name;meta\" (leave meta blank for zero metadata)");
        {
            int meta = 0;
            int weight = 0;
            for (String s : trash_things_arr) {
                String[] trash_wi = s.split("\\|");
                try {
        ...
        {
            int meta = 0;
            for (String s : fruitStacksArr) {
                String[] loc_meta = s.split(";");
                if (loc_meta.length > 1) {
                    try {
                        meta = Integer.parseInt(loc_meta[1]);
                    } catch (NumberFormatException e) {
