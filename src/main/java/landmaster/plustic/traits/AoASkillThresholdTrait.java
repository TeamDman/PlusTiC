...
public class AoASkillThresholdTrait extends AbstractTrait {
    ...
    public static AoASkillThresholdTrait fetch(Enums.Skills skill, int minLevel) {
        Int2ObjectMap<AoASkillThresholdTrait> lvToTrait = traitsCache.computeIfAbsent(skill, k -> new Int2ObjectOpenHashMap<>());
    ...
    public void onUpdate(ItemStack tool, World world, Entity entity, int itemSlot, boolean isSelected) {
        if (!world.isRemote && entity instanceof EntityPlayer && !((EntityPlayer) entity).capabilities.isCreativeMode && !PlayerUtil.doesPlayerHaveLevel((EntityPlayer) entity, getSkill(), getMinLevel())) {
            ...
            player.setHeldItem(player.getHeldItemOffhand() == tool ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, ItemStack.EMPTY);
