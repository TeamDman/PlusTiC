...
public class Global extends AbstractTrait {
    ...
    public void blockDrops(BlockEvent.HarvestDropsEvent event) {
        if (event.getWorld().isRemote || event.getHarvester() == null) return;
    ...
    private void __blockHarvestDrops(ItemStack tool, BlockEvent.HarvestDropsEvent event) {
        ...
        if (nbt0.hasKey("global", 10) && ToolHelper.isToolEffective2(tool, event.getState())) {
            ...
            if (te == null) return;
            IItemHandler ih = te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, EnumFacing.VALUES[nbt.getByte("facing")]);
            if (ih == null) return;
    ...
    public void dropEvent(LivingDropsEvent event) {
        ...
        if (world0.isRemote || event.getEntityLiving().getHealth() > 0) return;
    ...
    public void onPlayerUse(PlayerInteractEvent.RightClickBlock event) {
        ...
        event.getEntityPlayer().sendMessage(
                new TextComponentTranslation("msg.plustic.globalmodifier.set",
                coord.xCoord, coord.yCoord, coord.zCoord, coord.dimensionId)
        );
    }
