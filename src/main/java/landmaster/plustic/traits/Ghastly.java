...
public class Ghastly extends AbstractTrait {
    ...
    public void defend(LivingHurtEvent event) {
        if (event.getEntityLiving().isSneaking() && event.getSource() instanceof EntityDamageSource && Arrays.stream(EnumHand.values())
            ...
            Optional.ofNullable(event.getSource().getTrueSource()).ifPresent(attacker -> {
                if (attacker instanceof EntityLivingBase) {
                    ((EntityLivingBase) attacker).addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, 100, 2));
                }
            });
        }
    }
